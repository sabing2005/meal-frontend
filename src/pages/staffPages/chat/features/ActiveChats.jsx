...
const ActiveChats = ({ onChatSelect, selectedChat, clearedUnreadMap = {} }) => {
  ...
  // Fetch tickets from admin API to get ticket IDs
  const { data: ticketsResponse } = useGetTicketsQuery({
    page: 1,
    limit: 100
  });

  // Process orders data to create chat list
  useEffect(() => {
    if (ordersResponse?.data?.orders) {
      console.log('Processing orders, unreadCounts:', unreadCounts);
      
      // Create a map of order_id to ticket_id from tickets API
      const ticketMap = {};
      ...
      const processedChats = ordersResponse.data.orders
        ...
        .map((order, index) => {
        // Use API unread_messages as base, then add socket-based unread counts
        // If a chat has been cleared locally, always show 0 until a new customer message arrives
        ...
        return {
          ...
          ticketId: ticketMap[order.order_id] || `TKT-${order.order_id.slice(-6)}`, // Use actual ticket ID from tickets API
          orderId: order.order_id,
          order_id: order.order_id, // For socket compatibility
  ...
  useEffect(() => {
    // Connect to socket for real-time updates (only if not already connected)
    const connectionStatus = socketService.getConnectionStatus();
    console.log('🔌 Socket connection status:', connectionStatus);
    
    if (!connectionStatus.isConnected) {
      console.log('🔌 Connecting to socket...');
      socketService.connect('staff', 'all-orders');
    } else {
      console.log('🔌 Socket already connected');
    }

    // Get the socket instance to add comprehensive listeners
    const socket = socketService.socket;
    console.log('🔌 Socket instance:', socket);
    console.log('🔌 Socket connected?', socket?.connected);
    
    // Function to handle any message and update unread count
    const handleMessage = (messageData, source = 'unknown') => {
      ...
      // Only count messages from customers (not from admin/agent)
      ...
      // Try multiple possible orderId fields
    ...
    if (socket) {
      ...
      // Primary listener - this is the main event used by the chat system
      socket.on('chat.message', (data) => {
        handleMessage(data, 'chat.message');
      });
      
      // Backup listeners for other possible events
      ...
      // Also listen to any event that might contain messages
      ...
      // Test if socket is working by listening to connection events
      socket.on('connect', () => {
    ...
    // Use existing socket service methods as backup
        ...
        {activeChats.map((chat, index) => (
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mb-2">
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      ...
                      chat.status === 'placed' ? 'bg-blue-500/20 text-blue-400' :
                      chat.status === 'placed' ? 'bg-green-500/20 text-green-400' :
                      chat.status === 'cancelled' ? 'bg-red-500/20 text-red-400' :
              ...
              {/* Right Side - Metadata and Amount (Desktop) */}
              ...
              {/* Mobile Layout - Metadata and Amount */}
              <div className="sm:hidden">
                {/* Top row - All metadata in one line */}
                <div className="flex items-center gap-2 text-[10px] text-[#EDEDEDB2] font-inter mb-2">
                ...
                {/* Amount - Right aligned below the top row */}
