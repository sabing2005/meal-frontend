...
import StaffDetailsModal from "./features/StaffDetailsModal";
...
const StaffPage = () => {
  ...
  const sortedRoles = availableRoles.sort((a, b) => a.label.localeCompare(b.label));
  ...
  const [filters, setFilters] = useState({
    ...
    role: {
      ...
      options: [
        ...
        ...sortedRoles, // This will automatically include all roles in alphabetical order
      ],
    },
  });
  ...
  useEffect(() => {
    // Only show loading when search query actually changes (not on initial load)
  ...
  const handleAddStaff = async (newStaff) => {
    try {
      ...
      await createStaff(apiData).unwrap();
        ...
        <div className="bg-[#171D41] rounded-t-lg p-6">
          ...
          <div className="bg-[#171D41] rounded-lg border border-[#EDEDED33] p-4">
            <div className="flex items-center justify-center py-8">
              <PageLoading message="Loading staff members..." className="py-0" />
            </div>
          </div>
        </div>
      </div>
